...
<div class="_gaps">
	...
	<FormSlot>
		...
		<div class="_gaps_s">
			...
			<MkFolder v-if="matchQuery([i18n.ts._role._options.canFetchLBMetadata, 'canFetchLBMetadata'])">
				<template #label>{{ i18n.ts._role._options.canFetchLBMetadata }}</template>
				<template #suffix>
					<span v-if="role.policies.canFetchLBMetadata.useDefault" :class="$style.useDefaultLabel">{{ i18n.ts._role.useBaseValue }}</span>
					<span v-else>{{ role.policies.canFetchLBMetadata.value ? i18n.ts.yes : i18n.ts.no }}</span>
					<span :class="$style.priorityIndicator"><i :class="getPriorityIcon(role.policies.canFetchLBMetadata)"></i></span>
				</template>
				<div class="_gaps">
					<MkSwitch v-model="role.policies.canFetchLBMetadata.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts._role.useBaseValue }}</template>
					</MkSwitch>
					<MkSwitch v-model="role.policies.canFetchLBMetadata.value" :disabled="role.policies.canFetchLBMetadata.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts.enable }}</template>
					</MkSwitch>
					<MkRange v-model="role.policies.canFetchLBMetadata.priority" :min="0" :max="2" :step="1" easing :textConverter="(v) => v === 0 ? i18n.ts._role._priority.low : v === 1 ? i18n.ts._role._priority.middle : v === 2 ? i18n.ts._role._priority.high : ''">
						<template #label>{{ i18n.ts._role.priority }}</template>
					</MkRange>
				</div>
			</MkFolder>
