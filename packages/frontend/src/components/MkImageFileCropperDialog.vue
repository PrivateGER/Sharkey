...
// Crops a local image file before it is uploaded (unlike MkCropperDialog, which crops a drive file and uploads the result)
...
async function ok() {
	const blob = await new Promise<Blob>(async (res, rej) => {
		if (cropper == null) return rej(new Error('Cropper is not initialized'));
	...
	const finalFile = (props.imageFile instanceof File
		? new File([blob], props.imageFile.name, { type: blob.type })
		: blob) as F;
